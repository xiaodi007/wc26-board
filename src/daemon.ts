...
let lastPm = 0;
let lastKalshi = 0;
let lastWalrusAttempt = Date.parse(getMeta("walrus_latest_published_at") ?? "0") || 0;

log(`daemon start: PM every ${PM_POLL_MS / 60000}min, Kalshi every ${KALSHI_POLL_MS / 60000}min, Sporttery every ${SPORTTERY_POLL_MS / 60000}min, results every ${RESULTS_POLL_MS / 60000}min, OddsAPI every ${ODDSAPI_POLL_MS / 60000}min, Walrus publish every ${WALRUS_PUBLISH_MS / 60000}min${ODDS_API_KEY ? "" : " (no Odds API key)"}`);
...
async function tick(): Promise<void> {
  ...
  if (collected) {
    setMeta("daemon_last_collection_ts", new Date(now).toISOString());
  }

  if (WALRUS_ENABLED && now - lastWalrusAttempt >= WALRUS_PUBLISH_MS) {
    lastWalrusAttempt = now;
    try {
